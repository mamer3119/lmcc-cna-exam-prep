...
**Public repo:** https://github.com/mamer3119/lmcc-cna-exam-prep **Live site
(GitHub Pages):** https://mamer3119.github.io/lmcc-cna-exam-prep/ **Local dev:**
`pnpm dev:clean` → http://localhost:3005/lmcc-cna-exam-prep/

## What changed since last GitHub commit (verify on `main` after merge)

Phase 1 milestones M1–M4: mastery store, Learn polish, Sequence/Tolerance/Recall
drills, `useSyncExternalStore` instructor view, centralized practice labels,
Playwright hydration smoke.
...
| Framework landing page               | **Yes** | `app/framework/page.tsx`, `app/framework/pathway/page.tsx`           |
| Persistent 22-skill rail             | **Yes** | `components/SkillPathwayRail.tsx`, `app/skills/[slug]/layout.tsx`    |
| Core-only segment filter             | **Yes** | `?filter=core`, `lib/segment-filter.ts`, `SegmentFilterToggle.tsx`   |
...
Legacy `Study Mode` / `Quiz Mode` strings must **not** appear (grep gate:
`tests/practice-labels.test.ts`).
